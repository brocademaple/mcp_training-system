**文档版本**: v2.0（以 2.0 Multi-Agent + MCP 编排为主线）  
**创建日期**: 2026-01-10  
**最近更新**: 2026-05（双版本叙述：经典版为数据与操作面，Agent 版为 MCP 策划与编排）

> **说明**：下文**产品目标与架构**以 **2.0** 为默认叙述；**Agent 版**侧重 **MCP 协调下的训练任务策划与流水线执行**。**经典版**提供数据集与训练/评估的 **操作面**（手动逐步完成数据→训练→评估），数据与 Agent 版 **共用同一数据库与 API**；在对接叙述上，经典侧沉淀的数据可作为 **经 MCP 与 Agent 版衔接的来源**（独立 MCP Server 的形态由实现定义）。详细路线见 [`docs/PLANNING_2.0.md`](docs/PLANNING_2.0.md)、[`docs/SPEC_CODING.md`](docs/SPEC_CODING.md)。
...
构建 **Multi-Agent + MCP 编排** 的模型训练系统，使用户能够：

**Agent 版 / 一键流水线（2.0 默认体验）**

1. 准备或选择已清洗的训练数据（上传、URL 导入或沿用已有数据集）
2. 通过**流水线**一次性触发：**数据清洗 → 模型训练 → 模型评估**（由 Coordinator 按 MCP 消息编排三 Agent）
3. 查看流水线状态、训练任务、产出模型与评估报告；与经典版列表互通

**经典版（数据与操作面；可与 Agent 经 MCP 衔接）**

1. 上传数据集并完成清洗，任务与状态写入与 Agent 版相同的表
2. 创建训练任务、选择模型与超参
3. 创建评估任务并查看报告  
   上述结果可被 Agent 策划与流水线引用；若部署独立 MCP Server，可将经典侧上下文暴露给外部 Agent（可选）

两条路径**共用** `datasets`、`training_jobs`、`models`、`evaluations`、`pipeline_instances` 等数据；用户可通过界面**版本切换**在 Agent 版与经典版之间切换。

### 1.3 核心价值
+ **降低门槛**：非专业用户也能完成数据准备到评估的闭环
+ **2.0 编排**：一键流水线 + MCP 消息驱动多 Agent，流程可追溯
+ **双版本并存**：自动化编排与逐步操作同一套数据，不拆库
+ **可扩展**：在现有训练链路上支持文本分类头训练、SFT+LoRA 微调（`model_type` 区分）等；未来可接入顶层 Manager（见 `docs/PLANNING_2.0.md` 第九节）
+ **数据私有化**：支持本地与私有数据集
...
| 维度 | 2.0（主线） | 1.0 / 经典版（并存） |
|------|-------------|----------------------|
| **用户心智** | 「我要跑通一条训练链路」— 画布 / 流水线 / 智能编排入口 | 「我要逐步管理数据、任务、模型」— 各业务页逐项操作 |
| **编排** | `Coordinator` + MCP 消息串联 Data / Training / Evaluation Agent；`pipeline_instances` 记录会话与步骤 | 用户通过 HTTP 分别创建清洗结果、训练任务、评估任务 |
| **界面** | Agent 版：智能画布、流水线历史等 | 经典版：仪表盘（工作台）、数据集、训练、评估、模型等 |
| **数据** | 与经典版 **完全互通**（同一 PostgreSQL） | 同上 |

**已实现（2.0a 范畴）**：版本切换、`POST/GET /api/v1/pipelines`、流水线实例表、Agent 画布分步配置与启动流水线、经典版工作台「推荐下一步 / 最近动态」等（细节以当前代码与 `docs/PLANNING_2.0.md` 为准）。

**规划中（2.0b+）**：更细的可观测性、顶层自然语言 Manager（smolagents 等）、Skill/Workflow 模板等。
...
### 2.4 2.0 编排与双版本接入（当前实现）

在 **2.0** 下，除「用户逐步调 API / 经典版页面」外，增加 **流水线编排** 路径：

```plain
用户（Agent 版）选择数据集 + 训练配置 → POST /api/v1/pipelines
    ↓
PipelineHandler → Coordinator.RunPipeline
    ↓
异步：clean_data（Data Agent）→ train（Training Agent）→ evaluate（Evaluation Agent）
    ↓
pipeline_instances 更新 status / current_step / job_id / model_id / eval_id
```

**经典版**仍可直接调用数据集、训练、评估等 REST 接口，与流水线写入**同一套表**。**前端**通过 `app-version`（localStorage）切换 Agent / 经典布局与默认路由（详见 `frontend/src/App.tsx`、`Layout`）。
...
| 入口 | 说明 |
|------|------|
| **经典版** | 数据集管理、模型训练、模型评估、模型管理、流水线历史等；适合逐步操作与细粒度控制。 |
| **Agent 版** | 智能画布（意图与数据准备 → 训练计划 → 启动流水线）、流水线历史；一键触发 Coordinator 编排。 |
| **共用能力** | 数据清洗脚本、训练脚本（含 `text_classification` / `sft_finetune` 等 `model_type`）、评估与报告生成；进度经 Redis 推送、结果落 PostgreSQL。 |

以下 **5.1～5.3** 按 **Agent 能力**描述；调用方可为流水线中的 Coordinator，也可为经典版各 Handler 直接触发。
...
### 7.0 版本说明（以 2.0 为当前主线）

- **Phase 1～5**（下文）对应项目早期的 **1.0 交付节奏**，核心能力（数据、训练、评估、MCP 协调）**已完成**。
- **当前产品与 PRD 主线**为 **2.0**：在 1.0 能力之上，强调 **一键流水线**、**双版本 UI**、**pipeline_instances 与编排**、**工作台化仪表盘**；后续增强（可观测性、Manager、微调产品化等）见 [`docs/PLANNING_2.0.md`](docs/PLANNING_2.0.md)。
...
|------|------|
| 版本切换（经典 ⇄ Agent） | 已实现 |
| `POST/GET /api/v1/pipelines`、流水线状态查询 | 已实现 |
| `pipeline_instances`、异步编排 clean→train→eval | 已实现 |
| Agent 画布、流水线历史页 | 已实现（持续迭代） |
| 经典版工作台（推荐下一步、最近动态） | 已实现 |
| SFT+LoRA 微调脚本与 `model_type: sft_finetune` 路由 | 后端已具备，产品化入口见规划文档 |
| 顶层自然语言 Manager（smolagents 等） | 规划中 |

### Phase 1: 核心基础（Week 1-4）（1.0 时期）
**优先级 P0（最高）**:
...
### Phase 4: MCP 集成（Week 11-12）（1.0 时期）
**优先级 P0**:
...
### Phase 5: 优化与测试（Week 13-14）（1.0 时期）
**优先级 P1**:
...
### Phase 6: 2.0 Multi-Agent 与一键流水线（当前主线，持续迭代）

**目标**：在 MCP 协调层之上，提供产品化的一键流水线、双版本与可追溯的流水线实例。

**优先级 P0（核心）**：

1. ✅ 流水线 API 与 `pipeline_instances` 持久化
2. ✅ Coordinator 异步执行清洗 → 训练 → 评估
3. ✅ 前端版本切换与 Agent 画布 / 流水线历史入口
4. ⭕ 编排层可观测性（步骤日志、消息追溯）— 部分实现，持续增强

**优先级 P1**：

1. ⭕ 画布侧 Data Agent 规划选项与 `data_agent_prompt` 完整闭环
2. ⭕ 微调（SFT）在经典版 / 画布中的显式入口与文档一致

**交付物**：用户可在 Agent 版完成「选数据 → 启动流水线 → 查看任务/模型/评估」；经典版可见同源数据；与 `docs/PLANNING_2.0.md` 第八、九节对齐。
...
+ [ ] 训练完成后可以查看评估结果
+ [ ] 用户可以下载训练好的模型
+ [ ] **2.0**：用户可通过流水线一键完成清洗→训练→评估（Agent 版）
+ [ ] **2.0**：流水线实例可查询，与训练任务/模型/评估 ID 可关联
+ [ ] **2.0**：经典版与 Agent 版数据互通（同一数据集与任务可见）

### 技术要求
+ [ ] 三个 Agent 通过 MCP 协议通信（含 Coordinator 编排路径）
